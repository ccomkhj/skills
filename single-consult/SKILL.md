...
- **R2 (first review)** — `Agent(subagent_type: "claude", name: "reviewer", model: <--model, default fable>, prompt: <self-contained review brief>)`. B is fresh: the prompt must carry everything it needs (the question, your R1 proposal, and — for coding — an instruction to read `git diff` and run the test itself). B shares your working tree, so it sees your uncommitted changes directly.
...
- **Respond (odd interior rounds, A).** For each `Cn`: a verdict (`agree` / `partial` / `object`), the action you took, and reasoning when it's not pure agreement. Address **every** critique — skipping one is a bug. If you changed code, re-run the test and note the result.
- **Re-review (round `n-1`, only when `n >= 5`).** For each item A objected to or partial-applied, B decides `accept` or `double down`. **No fresh critiques** — a bug A introduced in R3 is a double-down with a sub-finding, not a new C. B's last word.
- **Synthesize (final round, A).** Write the user-facing close: what you landed on, where you agreed, unresolved tensions stated plainly, and what you need from the user. Then stop and ask.
...
| Adding new critiques in R4 | R4 is `accept` / `double down` only. |
