...
| [humanizer](./humanizer) | 2.8.1 | Remove signs of AI-generated writing (significance inflation, em dashes, rule of three, AI vocabulary, filler, …) to make text sound human; based on Wikipedia's "Signs of AI writing" | Source from [blader/humanizer](https://github.com/blader/humanizer), improved here |
...
| [dual-haul](./dual-haul) | 1.0.0 | **Plugin (6 skills).** Goal-driven improvement loop: understand what needs improving → forge a `/goal` completion condition → each `/goal` turn, race a different-model Claude against Codex in separate git worktrees and merge the measured winner → dual-report → open a PR. Shared state in `.dualhaul/` | — |
