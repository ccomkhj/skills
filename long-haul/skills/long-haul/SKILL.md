...
1. `haul-spec` — turn the ask into a well-defined **spec** with a transcript-demonstrable success signal and a declared **toolbox** (the skills + MCP the haul may use). If the ask is vague, grill it first.
2. `haul-goal` — forge a `/goal`-ready completion condition from the spec. **GATE: user approves + runs `/goal`.**
...
4. `haul-ship` — once the goal clears (or the cap trips), report goal-vs-achieved and open a PR. **GATE: user confirms.**
...
clears — then proceed to `haul-ship`.
...
digraph long_haul {
  ...
  "haul-spec -> SPEC.md" [shape=box];
  "haul-goal -> GOAL.md" [shape=box];
  ...
  "haul-ship -> SUMMARY.md + PR" [shape=doublecircle];

  "Init .longhaul/, record PHASE" -> "haul-spec -> SPEC.md";
  "haul-spec -> SPEC.md" -> "haul-goal -> GOAL.md";
  "haul-goal -> GOAL.md" -> "GATE: user approves + runs /goal";
  ...
  "Goal met? (/goal cleared)" -> "haul-ship -> SUMMARY.md + PR" [label="yes (or cap hit)"];
}
```
...
3. **Drive the phases in order**, invoking each sibling skill. Update `PHASE` as you cross each boundary so a re-invocation resumes cleanly. If the user gave no ask, don't enumerate the spec questions yourself — go straight into `haul-spec`, which collects them via tabbed `AskUserQuestion` calls.
...
5. **Wrap-up deps.** `haul-ship` opens a PR, so verify now, not at the end: `gh` present + authed (`gh auth status`), `REPO`'s remote reachable, push permission. Missing → soft-flag "blocks at haul-ship".
...
|---|---|---|
| **approve-goal** | after `haul-goal` | Show `GOAL.md`'s condition and the `/goal …` line. Wait for the user to edit/approve and to actually run `/goal`. Do not start hauling until the goal is active. |
| **confirm-pr** | in `haul-ship` | Show the branch name + commit list + PR body. Only push/open after the user confirms — *unless* `GATES` sets `confirm_pr=auto`. |
...
(default — `haul-ship` parks for a yes) or `auto` (for an unattended finish —
`haul-ship` opens a *draft* PR without parking and never pushes to a protected or
...
- `haul-spec` writes `SPEC.md` (the deliverable, the success signal, the toolbox, constraints).
- `haul-goal` writes `GOAL.md` (+ the `/goal` line) from `SPEC.md`.
- `haul-loop` runs one explore-or-exploit round per `/goal` turn against `GOAL.md`, writes `R<N>.md`, and ratchets the `INCUMBENT`.
- `haul-ship` writes `SUMMARY.md`, then commits on a branch and opens the PR.

State-file templates live in [reference/file-formats.md](reference/file-formats.md).

## Termination & escape hatches

- **Normal:** goal clears → `haul-loop` sets `STATUS: GOAL-MET` → `haul-ship` → user confirms → PR opened → `STATUS: DONE`.
- **Round cap hit, or the well is dry** (`haul-loop`'s own stopping conditions): it sets `STATUS: WAITING-USER: cap-decision` and surfaces a partial against the incumbent. Decide with the user whether to raise `ROUNDS`, accept (→ `haul-ship`), or stop. (Definitions live in `haul-loop`; don't restate them here.)
...
- `ACTIVE: orchestrator` → resume the phase named by `PHASE` (`spec`→haul-spec, `goal`→haul-goal, `haul`→haul-loop, `wrap`→haul-ship) from the top; the phases are re-entrant (e.g. a haul interrupted mid-grilling just re-asks).
...
- `GOAL-MET` → the loop is done and the goal holds; go straight to `haul-ship`.
