# pair-goal worktree-race + human-intervention helpers.
#
# Source once (from the repo root, with .pairgoal/ present):
#   source ~/.claude/skills/pair-goal/reference/handoff.sh   # Claude Code orchestrator
#   source ~/.agents/skills/pair-goal/reference/handoff.sh   # (rarely needed; racers get a self-contained brief)
#
# THE MODEL
# ---------
# Unlike the other pair-* skills (one peer, ping-pong rounds), pair-goal runs a
...
#
# Functions (orchestrator-side, called inside a pair-iterate round):
...
#                              .pairgoal/R<round>-brief.md. Logs to
#                              .pairgoal/round-<round>-<name>.log + session.log.
#   goal_wait_race <round>   — block until BOTH racers drop their .pairgoal-done
...
#
# Human-side:
#   goal_watch               — tail -F .pairgoal/session.log (survives round boundaries).
...
_goal_refuse_if_racer() {
  if [[ "${PAIR_GOAL_ROLE:-}" == "racer" ]]; then
    echo "$1: you are a RACER — implement in your worktree, commit, touch .pairgoal-done, and EXIT." >&2
...
goal_race() {
  ...
  if [[ ! -f .pairgoal/STATE.md ]]; then
    echo "goal_race: no .pairgoal/STATE.md in $(pwd)" >&2
    return 2
  fi
  local brief=".pairgoal/R${round}-brief.md"
  if [[ ! -s "$brief" ]]; then
  ...
  local cmodel xmodel
  cmodel=$(awk '/^RACER_CLAUDE:/ {sub(/^RACER_CLAUDE:[[:space:]]*/,""); print; exit}' .pairgoal/STATE.md)
  xmodel=$(awk '/^RACER_CODEX:/  {sub(/^RACER_CODEX:[[:space:]]*/,"");  print; exit}' .pairgoal/STATE.md)
  ...
  local session_log=".pairgoal/session.log"
  ...
  local wt_claude=".pairgoal/wt-claude" wt_codex=".pairgoal/wt-codex"
  if ! git worktree add -q -b "pairgoal/r${round}-claude" "$wt_claude" HEAD 2>>"$session_log"; then
    echo "goal_race: failed to create $wt_claude worktree (branch pairgoal/r${round}-claude exists?). See $session_log." >&2
    return 4
  fi
  if ! git worktree add -q -b "pairgoal/r${round}-codex" "$wt_codex" HEAD 2>>"$session_log"; then
    echo "goal_race: failed to create $wt_codex worktree." >&2
    git worktree remove --force "$wt_claude" 2>/dev/null; git branch -D "pairgoal/r${round}-claude" 2>/dev/null
    return 4
  fi
  ...
  local promptfile=".pairgoal/R${round}-prompt.txt"
  cp "$brief" "$promptfile"
  cat >> "$promptfile" <<EOF
...
3. Commit: git add -A && git commit -m "pairgoal r${round} racer".
4. AFTER committing, mark done: touch .pairgoal-done
Then STOP. Never push, never edit anything outside this directory, never touch the parent repo's .pairgoal directory.
EOF
  ...
  local clog=".pairgoal/round-${round}-claude.log"
  ...
  local xlog=".pairgoal/round-${round}-codex.log"
...
# goal_wait_race <round> — block until BOTH racers drop .pairgoal-done.
...
goal_wait_race() {
  ...
  local cdone=".pairgoal/wt-claude/.pairgoal-done"
  local xdone=".pairgoal/wt-codex/.pairgoal-done"
  local clog=".pairgoal/round-${round}-claude.log"
  local xlog=".pairgoal/round-${round}-codex.log"
  ...
  while (( elapsed < timeout )); do
    ...
    if (( elapsed >= interval * 4 )) && command -v lsof >/dev/null 2>&1; then
      ...
      if [[ -n "$who" ]]; then
        echo "goal_wait_race: racer(s)$who may have died (no process, stale log, no .pairgoal-done)." >&2
...
goal_teardown() {
  ...
  if [[ -n "$round" ]]; then
    for name in claude codex; do
      git branch -D "pairgoal/r${round}-${name}" 2>/dev/null
    done
  fi
  return 0
}
...
goal_watch() {
  [[ -d .pairgoal ]] || { echo "goal_watch: no .pairgoal/ in $(pwd)" >&2; return 2; }
  touch .pairgoal/session.log
  echo "goal_watch: current state:"
  grep -E '^(PHASE|STATUS|ROUND|ROUNDS|RACER_CLAUDE|RACER_CODEX):' .pairgoal/STATE.md 2>/dev/null | sed 's/^/  /'
  echo "--- live feed (Ctrl-C to exit) ---"
  tail -F .pairgoal/session.log
}

goal_status() {
  [[ -f .pairgoal/STATE.md ]] || { echo "goal_status: no .pairgoal/STATE.md in $(pwd)" >&2; return 2; }
  echo "=== state ==="
  grep -E '^(PHASE|STATUS|ROUND|ROUNDS|RACER_CLAUDE|RACER_CODEX):' .pairgoal/STATE.md | sed 's/^/  /'
  echo; echo "=== user notes ==="
  if [[ -f .pairgoal/USER_NOTES.md ]]; then
    grep -E '^## N' .pairgoal/USER_NOTES.md 2>/dev/null | sed 's/^/  /' || echo "  (none)"
  else echo "  (none)"; fi
  echo; echo "=== last 20 lines of session.log ==="
  [[ -s .pairgoal/session.log ]] && tail -20 .pairgoal/session.log | sed 's/^/  /' || echo "  (no activity yet)"
}

goal_inject() {
  local msg="$*"
  [[ -n "$msg" ]] || { echo "goal_inject: usage: goal_inject \"<note>\"" >&2; return 2; }
  [[ -d .pairgoal ]] || { echo "goal_inject: no .pairgoal/ in $(pwd)" >&2; return 2; }
  local f=".pairgoal/USER_NOTES.md"
  [[ -f "$f" ]] || { echo "# User notes (mid-flight)"; echo; } > "$f"
  local n; n=$(grep -oE '^## N[0-9]+' "$f" | sed 's/^## N//' | sort -n | tail -1); n=$((${n:-0}+1))
  local round=""; [[ -f .pairgoal/STATE.md ]] && round=$(awk '/^ROUND:/ {print $2; exit}' .pairgoal/STATE.md)
...
goal_takeover() {
  [[ -d .pairgoal ]] || { echo "goal_takeover: no .pairgoal/ in $(pwd)" >&2; return 2; }
  local pids=""
  command -v lsof >/dev/null 2>&1 && pids=$(lsof -t .pairgoal/round-*.log 2>/dev/null | sort -u)
  if [[ -z "$pids" ]]; then
    echo "goal_takeover: no racer currently writing .pairgoal/round-*.log"
  else
  ...
  if [[ -f .pairgoal/STATE.md ]]; then
    sed -i.bak 's/^STATUS: .*/STATUS: BLOCKED: human-takeover/' .pairgoal/STATE.md; rm -f .pairgoal/STATE.md.bak
