...
2. **Branch.** If the work landed on `main`/`master`, create a feature branch (e.g. `pairgoal/<short-topic>`) and move the commits there — never open a PR from the default branch. If already on a feature branch, use it.
...
- Don't include `.pairgoal/` or the `pairgoal/r<N>-*` branches in the PR. They're transient.
