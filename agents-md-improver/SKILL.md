...
**Write improvements into `AGENTS.md`, not `CLAUDE.md`** — keep `CLAUDE.md` as a one-line `@AGENTS.md` reference so the same project memory is shared across tools (Claude Code, Codex, Gemini CLI, …); everywhere below that reads or writes "CLAUDE.md", treat `AGENTS.md` as the write target (creating it, and the `@AGENTS.md` pointer, if absent).
